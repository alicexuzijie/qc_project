...
class QualityControlRoutine():
    ...
    def init_non_device(self):
        qc_devices_df = self.device_list_info[~self.device_list_info['RELATE_SITE_ID'].isin([-1])]
        qc_devices = qc_devices_df['DEV_ID'].unique().tolist()
        for key in self.dic_city.keys():
            self.non_devices[key] = set(self.dic_city[key]).difference(set(qc_devices))
    ...
    def get_no_data_devices(self, var):
        ...
        :return:
        """
        if not var in self.non_devices.keys():
            return []
        #获取没有数据的dev_id
        ...
        # self.non_devices[var] = no_data_devices_df['DEV_ID'].unique().tolist()
        self.non_devices[var] = set(self.non_devices[var]).difference(set(var_devices))
        return self.non_devices[var]
    ...
    def get_no_data_devices_for_site_interpolate(self, var):
        ...
        #邻域插值后还没有的设备列表
        if var in self.non_devices.keys():
            devices_for_site_interpolate = set(self.non_devices[var]).difference(set(interpolate_dev_list))
        else:
            #没有需要插值的
            devices_for_site_interpolate = []
